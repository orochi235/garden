...
describe('expandToGroups', () => {
  it('passes through ungrouped ids unchanged', () => {
    const a = makeStructure({ type: 'bed', x: 0, y: 0, width: 1, height: 1, groupId: null });
    const b = makeStructure({ type: 'bed', x: 2, y: 0, width: 1, height: 1, groupId: null });
    expect(expandToGroups([a.id], [a, b])).toEqual([a.id]);
  });

  it('expands a single grouped member to all siblings', () => {
    const a = makeStructure({ type: 'bed', x: 0, y: 0, width: 1, height: 1, groupId: 'g1' });
    const b = makeStructure({ type: 'bed', x: 2, y: 0, width: 1, height: 1, groupId: 'g1' });
    const c = makeStructure({ type: 'bed', x: 4, y: 0, width: 1, height: 1, groupId: 'g1' });
  ...
  it('handles multiple groups', () => {
    const a = makeStructure({ type: 'bed', x: 0, y: 0, width: 1, height: 1, groupId: 'g1' });
    const b = makeStructure({ type: 'bed', x: 2, y: 0, width: 1, height: 1, groupId: 'g1' });
    const c = makeStructure({ type: 'bed', x: 4, y: 0, width: 1, height: 1, groupId: 'g2' });
    const d = makeStructure({ type: 'bed', x: 6, y: 0, width: 1, height: 1, groupId: 'g2' });
    const e = makeStructure({ type: 'bed', x: 8, y: 0, width: 1, height: 1, groupId: null });
  ...
  it('is idempotent', () => {
    const a = makeStructure({ type: 'bed', x: 0, y: 0, width: 1, height: 1, groupId: 'g1' });
    const b = makeStructure({ type: 'bed', x: 2, y: 0, width: 1, height: 1, groupId: 'g1' });
    const c = makeStructure({ type: 'bed', x: 4, y: 0, width: 1, height: 1, groupId: null });
