...
describe('passesAlmanacFilters', () => {
  ...
  describe('cellSizes', () => {
    it('passes when cell size is in filter', () => {
      const c = makeCultivar({
        seedStarting: { ...DEFAULT_SEED_STARTING_FIELDS, cellSize: 'small' },
      });
      expect(
        passesAlmanacFilters(c, makeSpecies(), { ...emptyFilters, cellSizes: ['small', 'medium'] }),
      ).toBe(true);
    });

    it('fails when cell size not in filter', () => {
      const c = makeCultivar({
        seedStarting: { ...DEFAULT_SEED_STARTING_FIELDS, cellSize: 'large' },
      });
      expect(
        passesAlmanacFilters(c, makeSpecies(), { ...emptyFilters, cellSizes: ['small'] }),
      ).toBe(false);
    });
    ...
    it('cultivar override wins over species default', () => {
      ...
      expect(passesAlmanacFilters(c, species, { ...emptyFilters, cellSizes: ['small'] })).toBe(
        true,
      );
    });
  });
  ...
  describe('seasons', () => {
    it('passes when species season matches', () => {
      ...
      expect(passesAlmanacFilters(makeCultivar(), s, { ...emptyFilters, seasons: ['cool'] })).toBe(
        true,
      );
    });
    ...
    it('passes when species has no seasons declared (missing data passes)', () => {
      expect(
        passesAlmanacFilters(makeCultivar(), makeSpecies(), { ...emptyFilters, seasons: ['cool'] }),
      ).toBe(true);
    });
  });
  ...
  describe('usdaZone', () => {
    ...
    it('passes when species has no usdaZones (missing data passes)', () => {
      expect(
        passesAlmanacFilters(makeCultivar(), makeSpecies(), { ...emptyFilters, usdaZone: 6 }),
      ).toBe(true);
    });
  });
  ...
  describe('lastFrostDate', () => {
    it('passes when today falls within sow window', () => {
      ...
      expect(passesAlmanacFilters(c, makeSpecies(), { ...emptyFilters, lastFrostDate: iso })).toBe(
        true,
      );
    });
    ...
    it('passes when cultivar has no weeksBeforeLastFrost (missing data passes)', () => {
      const c = makeCultivar();
      expect(
        passesAlmanacFilters(c, makeSpecies(), { ...emptyFilters, lastFrostDate: '2026-05-01' }),
      ).toBe(true);
    });
  });
