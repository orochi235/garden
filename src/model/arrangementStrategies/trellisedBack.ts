...
export function computeTrellisedBack(
  ...
  if (trellisRect && config.trellisPitchFt > 0) {
    ...
    const center = along
      ? trellisRect.y + trellisRect.length / 2
    ...
    const end = along ? trellisRect.x + trellisRect.width : trellisRect.y + trellisRect.length;
...
function trellisBand(b: ParentBounds, edge: 'N' | 'E' | 'S' | 'W', depth: number, m: number): ParentBounds | null {
  const inner = { x: b.x + m, y: b.y + m, w: b.width - 2 * m, h: b.length - 2 * m };
  if (inner.w <= 0 || inner.h <= 0) return null;
  switch (edge) {
    case 'N': return { x: inner.x, y: inner.y, width: inner.w, length: Math.min(depth, inner.h), shape: 'rectangle' };
    case 'S': return { x: inner.x, y: inner.y + Math.max(0, inner.h - depth), width: inner.w, length: Math.min(depth, inner.h), shape: 'rectangle' };
    case 'W': return { x: inner.x, y: inner.y, width: Math.min(depth, inner.w), length: inner.h, shape: 'rectangle' };
    case 'E': return { x: inner.x + Math.max(0, inner.w - depth), y: inner.y, width: Math.min(depth, inner.w), length: inner.h, shape: 'rectangle' };
  }
}
...
function frontBand(b: ParentBounds, edge: 'N' | 'E' | 'S' | 'W', depth: number, m: number): ParentBounds | null {
  ...
  if (inner.w <= 0 || inner.h <= 0) return null;
  switch (edge) {
    case 'N': return { x: inner.x, y: inner.y + depth, width: inner.w, length: Math.max(0, inner.h - depth), shape: 'rectangle' };
    case 'S': return { x: inner.x, y: inner.y, width: inner.w, length: Math.max(0, inner.h - depth), shape: 'rectangle' };
    case 'W': return { x: inner.x + depth, y: inner.y, width: Math.max(0, inner.w - depth), length: inner.h, shape: 'rectangle' };
    case 'E': return { x: inner.x, y: inner.y, width: Math.max(0, inner.w - depth), length: inner.h, shape: 'rectangle' };
  }
}
...
function runFront(strategy: 'rows' | 'square-foot' | 'hex', rect: ParentBounds, cultivars?: Cultivar[]): Slot[] {
  switch (strategy) {
    case 'rows': {
      const out: Slot[] = [];
      const pitch = 0.5;
      for (let y = rect.y + pitch / 2; y <= rect.y + rect.length; y += pitch) {
