...
describe('useMoveInteraction', () => {
  ...
  it('moves a structure', () => {
    ...
    result.current.move(mouseEvent(8, 7));
  ...
  it('moves child structures along with parent', () => {
    useGardenStore.getState().addStructure({ type: 'patio', x: 5, y: 5, width: 6, height: 6 });
    const patio = useGardenStore.getState().garden.structures[0];
    ...
    // Start dragging the patio from world (6,6), move mouse by (3,2)
    result.current.start(6, 6, patio.id, 'structures', patio.x, patio.y);
    result.current.move(mouseEvent(9, 8));
    ...
    // Patio moved by (3,2)
    expect(updatedPatio.x).toBe(8);
    expect(updatedPatio.y).toBe(7);
    // Pot moved by same delta
    expect(updatedPot.x).toBe(9);
    expect(updatedPot.y).toBe(8);
  });

  it('moves multiple children along with parent', () => {
    useGardenStore.getState().addStructure({ type: 'patio', x: 2, y: 2, width: 8, height: 8 });
    const patio = useGardenStore.getState().garden.structures[0];
    ...
    // Both pots moved by (2,2)
    expect(updatedPot1.x).toBe(5);
    expect(updatedPot1.y).toBe(5);
    expect(updatedPot2.x).toBe(7);
    expect(updatedPot2.y).toBe(6);
  });
  ...
  it('does not move unrelated structures', () => {
    ...
    result.current.move(mouseEvent(5, 5));
  ...
  it('child positions are stable across multiple move events', () => {
    useGardenStore.getState().addStructure({ type: 'patio', x: 0, y: 0, width: 10, height: 10 });
    const patio = useGardenStore.getState().garden.structures[0];
    ...
    result.current.move(mouseEvent(2, 2));
    result.current.move(mouseEvent(3, 3));
    result.current.move(mouseEvent(4, 4));

    const structures = useGardenStore.getState().garden.structures;
    const updatedPatio = structures.find((s) => s.id === patio.id)!;
    const updatedPot = structures.find((s) => s.id === pot.id)!;

    // Total delta is (3,3) — no cumulative rounding drift
    expect(updatedPatio.x).toBe(3);
    expect(updatedPatio.y).toBe(3);
    expect(updatedPot.x).toBe(5);
    expect(updatedPot.y).toBe(5);
  });
  ...
  it('clears child tracking on end', () => {
    ...
    expect(finalPatio.x).toBe(2);
    expect(finalPatio.y).toBe(2);
  });
});
