  ...
  return (
    <div className={f.panel}>
      ...
      <div className={f.grid}>
        ...
        <input className={`${f.input} ${f.span4}`} type="number" step="0.1" min="0.1"
        ...
        <input className={f.colorInput} type="color" value={obj.color.slice(0, 7)}
          onChange={(e) => updateObj({ color: e.target.value })} />

        {structure && (
          <>
            <span className={f.label}>Type</span>
            <select className={`${f.select} ${f.span12}`} value={structure.type}
        ...
        <button className={`${f.input} ${f.span12}`} style={{ cursor: 'pointer', textAlign: 'center', color: 'var(--color-terracotta)' }}
          onClick={() => {
