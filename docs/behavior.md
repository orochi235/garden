...
- Structure and zone moves snap to the garden's grid cell size; holding Alt while moving bypasses the snap. Plantings ignore this snap (their pose comes from the container's layout strategy). A structure with `snapToGrid: false` skips grid-snap entirely and moves freely; zones have no such flag and always default to grid-snap.
- Kit-driven drag-and-drop reparenting of a planting (via `gardenSceneAdapter.setParent`) preserves the visual world position: local coords are recomputed from the current world pose and passed to `updatePlanting` with `skipRearrange: true`, bypassing `rearrangePlantings`. All other callers (palette drop, arrangement edits) continue to invoke `rearrangePlantings` as before.
...
- Garden palette → canvas drag uses the same `palettePointerPayload` handoff slot in `useUiStore`. The garden canvas's `useGardenPaletteDropTool` watches the slot, owns the ghost icon + threshold drag + drag overlay + commit, and computes world coords by reading `useUiStore.zoom`/`panX`/`panY` directly (garden mode keeps the viewport in the ui store as the single source of truth — full canvas-owned view migration is deferred). `App.handlePaletteDragBegin` is a 3-line setter that just writes the payload
