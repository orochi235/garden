...
- ~~**Hardiness-zone data source is wrong.**~~ Resolved: `scripts/build-frost-zone-grid.mjs` now ingests the official USDA PHZM 2023 BIL rasters from PRISM (CONUS + AK + HI + PR) for the zone field, alongside NOAA `ANN-TMIN-PRBLST-T32FP50` for last-frost. Resampled to the existing 0.5° grid by averaging in-bounds PHZM pixels per cell. Verified Boston→6b, Denver→6a, Miami→11a, Seattle→9a. Zone auto-fill re-enabled in `AlmanacPanel.tsx`. Per PRISM/OSU terms, any rendered map derived from PHZM must carry a "not the official USDA Plant Hardiness Zone Map" disclaimer (we don't render maps today; consider when/if we do).
- **Frost-zone grid coverage beyond CONUS + AK + HI + PR.** `public/data/frost-zone-grid.bin` now covers PHZM regions (CONUS, AK, HI, PR) for hardiness zone, but last-frost still only comes from the NOAA U.S. station network and the lat/lon box is 17..72 N, -172..-65 E. Users in Guam, USVI, Canada, or anywhere else get "Couldn't determine zone for this location." Future: integrate an additional dataset (e.g. NRCan Plant Hardiness for Canada, EU Köppen-derived zones) and widen the grid bounds — or fall back to a coarser global Köppen lookup when off-grid.
