...
- [ ] **HiGHS LP-string parse error: "Unable to parse solution. Too few lines."** Surfaced after removing the silent `try/catch` in `src/optimizer/worker.ts`. Three iterations all throw the same error, leaving `optimizerResult.candidates` empty so no candidate UI appears. Likely either (a) `mipModelToLpString` produces output HiGHS can't accept, (b) HiGHS LP solver returns truncated output that the npm wrapper can't parse, or (c) the model is producing pathological constraints. Reproduce by selecting `default-bed-1` (the tomato bed) and clicking Solve. Investigate by dumping the LP string, running it against a standalone HiGHS binary, and inspecting the npm package's parser.
