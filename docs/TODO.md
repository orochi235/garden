...
- ~~Per-id flash opacity for seedling selection still unwired; garden currently aggregates all selected ids into a single `highlightOpacity`. Push per-id pulses into both modes.~~ Resolved on the seedling side: `SeedStartingCanvasNewPrototype` passes `useHighlightStore.getState().computeOpacity` straight through to `createSeedlingLayers`, and the layer applies it per seedling. Garden mode is being refactored separately on `worktree-agent-a8bd6184`.
