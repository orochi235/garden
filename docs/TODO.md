...
- **Sibling z-order.** *Spec + plan written; ready to implement.* See `docs/superpowers/specs/2026-05-01-canvas-kit-sibling-zorder-design.md` and `docs/superpowers/plans/2026-05-01-canvas-kit-sibling-zorder.md`. Approach: implicit array order via adapter (`getChildren(parentId)` returns ids in z-order), not a per-object `zIndex` field. Adds reorder ops (`bringForward`, `sendBackward`, `bringToFront`, `sendToBack`, `moveToIndex`) and a `useReorderAction` hook (`]` / `[` and bracket-with-shift).
